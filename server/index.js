...
var io = require("socket.io").listen(10012);
            ...
            if(type == 'message'){
                db.query('SELECT seudo, socket from users WHERE user = ?', [to], function(err, rows, fields) {
                  if (rows.length > 0){
                    ...
                    var myDate = moment().format("YYYY-MM-D HH:mm:ss"); 
                    var mensaje = '{ "mensa": "'+msg.mensaje+'", "name": "'+msg.name+'", "from": "'+msg.from+'", "to": "'+to+'", "seudo": "'+seudo+'", "type": "'+type+'" }';
                        
                    io.to(para).emit("msg", nick, mensaje);
                    db.query('INSERT INTO mensajes values (?,?,?,?,?,?)',  ['',nick,to,msg.mensaje,myDate,1], function(err, result) {
                        if (err){ 
                            console.log(err);
                        }else{
